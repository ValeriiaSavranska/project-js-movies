    ...
    .map(({ poster_path, backdrop_pathL, genre_ids, release_date, title }) => {
      ...
      getGenres().then(data => {
        for (let i = 0; i < genre_ids.length; i++) {
          const keys = Object.keys(data.genres);
          for (const key of keys) {
            // console.log(genre_ids[i]);
            // console.log(data.genres[key].id);
            if (genre_ids[i] === data.genres[key].id) {
              arrGenre.push(data.genres[key].name);
              str = str + data.genres[key].name;
              console.log(str);
            }
          }
        }
